...
@section('title', 'Edit Troubleshoot Error Code')
...
@section('content')
        ...
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-sm-6 text-start">
                    <h1 class="page-title"><i class="fas fa-edit me-2"></i> Edit Troubleshoot Code</h1>
                    <p class="text-muted small mb-0 mt-1">Update diagnostic error code information.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="form-card">
                <form action="{{ route('troubleshoots.update', $troubleshoot->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-12 mb-4 text-start">
                            ...
                            <input type="text" class="form-control custom-input @error('name') is-invalid @enderror"
                                name="name" value="{{ old('name', $troubleshoot->name) }}"
                                placeholder="Enter troubleshoot error code name" required>
                            @error('name')
                        ...
                        <div class="col-md-12 mb-4 text-start">
                            ...
                            <select class="form-control custom-input @error('status') is-invalid @enderror" name="status"
                                required>
                                <option value="active"
                                    {{ old('status', $troubleshoot->status) == 'active' ? 'selected' : '' }}>Active</option>
                                <option value="inactive"
                                    {{ old('status', $troubleshoot->status) == 'inactive' ? 'selected' : '' }}>Inactive
                                </option>
                            </select>
                            @error('status')
                                <span class="invalid-feedback">{{ $message }}</span>
                    ...
                    <div class="mt-4 text-start border-top pt-4">
                        <button type="submit" class="btn-submit-custom">
                            <i class="fas fa-save me-1"></i> Update Error Code
                        </button>
