...
@section('content')
    ...
    <div class="content-header px-4 pt-4 text-start">
        <div class="container-fluid">
            <h1 class="page-title"><i class="fas fa-upload me-2"></i> {{ isset($video) ? 'Edit Video' : 'Add New Video' }}
            </h1>
        </div>
    </div>
                        ...
                        <div class="form-card">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="d-flex gap-4">
                                    <label class="d-flex align-items-center"><input type="radio" name="video_type"
                                            value="link" class="me-2"
                                            {{ old('video_type', $video->video_type ?? 'link') == 'link' ? 'checked' : '' }}>
                                        YouTube/Vimeo Link</label>
                                    <label class="d-flex align-items-center"><input type="radio" name="video_type"
                                            value="upload" class="me-2"
                            ...
                            <div id="video_url_section" class="mb-4">
                                ...
                                <input type="text" id="video_url" name="video_url" class="form-control custom-input"
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-4">
                                    ...
                                    <input type="text" name="duration" class="form-control custom-input"
                        ...
                        <div class="sidebar-card">
                            <div class="mb-4">
                                ...
                                <select name="status" class="form-select custom-input">
                                    <option value="published"
                                        {{ old('status', $video->status ?? '') == 'published' ? 'selected' : '' }}>Published
                                    </option>
                                    <option value="draft"
                                        {{ old('status', $video->status ?? '') == 'draft' ? 'selected' : '' }}>Draft
                                    </option>
                                    <option value="archived"
                                        {{ old('status', $video->status ?? '') == 'archived' ? 'selected' : '' }}>Archived
                                    </option>
                                </select>
                            </div>
                            ...
                            <div id="thumbnail-preview" class="text-center mt-3">
                                ...
                                <img id="preview-img"
                                    ...
                                    style="max-height: 150px; "{{ !isset($video->thumbnail) ? 'display:none;' : '' }}">
                            </div>
                        </div>

                        <div class="mt-4">
                            <button type="submit"
                                class="btn-submit-custom w-100">{{ isset($video) ? 'Update Video' : 'Add Video to Library' }}</button>
                            <a href="{{ route('videos.index') }}"
                                class="btn btn-link w-100 text-muted fw-bold mt-2 text-decoration-none">Cancel</a>
                        </div>
                    </div>
                </div>
            </form>
    ...
    <script>
        function toggleVideoSections() {
            ...
            if (videoType === 'link') {
                ...
                $('#video_file_section').hide();
            } else {
                $('#video_url_section').hide();
                $('#video_file_section').show();
            }
        }
